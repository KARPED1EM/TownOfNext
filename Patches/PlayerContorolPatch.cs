    ...
    public static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
    {
        ...
        switch (target.GetCustomRole())
        {
            ...
            case CustomRoles.Imposterr:
                if (Main.ImposterrProtect.ContainsKey(target.PlayerId))
                    if (Main.ImposterrProtect[target.PlayerId] + Options.ImposterrSkillDuration.GetInt() >= Utils.GetTimeStamp(DateTime.Now))
                    {
                        killer.SetRealKiller(target);
                        target.RpcMurderPlayer(killer);
                        Logger.Info($"{target.GetRealName()} 反弹：{killer.GetRealName()}", "Imposterr Kill");
                        return false;
                    }
                break;
        }
...
class MurderPlayerPatch
{
    ...
    class EnterVentPatch
    {
        ...
        if (pc.Is(CustomRoles.Imposterr))
            {
            Main.ImposterrProtect.Remove(pc.PlayerId);
            Main.ImposterrProtect.Add(pc.PlayerId, Utils.GetTimeStamp(DateTime.Now));
            new LateTask(() =>
            {
                if (GameStates.IsInTask && !GameStates.IsMeeting) pc.RpcGuardAndKill(pc);
            }, 0.5f, "Imposterr Protect");
        }
