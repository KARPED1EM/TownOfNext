...
class ChatCommands
{
    ...
    public static bool MafiaMsgCheck(PlayerControl pc, string msg)
    {
        ...
        if (msg == "/rv")
        {
            ...
            foreach (var npc in PlayerControl.AllPlayerControls)
            {
                ...
                text += "\n" + npc.PlayerId.ToString() + " → (" + npc.GetDisplayRoleName() + ") " +
                        npc.GetRealName();
            }
        ...
        Utils.SendMessage(Name + " " + GetString("MafiaKillSucceed"), 255,
            Utils.ColorString(Utils.GetRoleColor(CustomRoles.Mafia), " ★ 特供情报 ★ "));
    ...
    public static bool ProhibitedCheck(PlayerControl player, string text)
    {
        ...
        if (Options.AutoKickStart.GetBool())
        {
            if (ContainsStart(text) && GameStates.IsLobby)
            {
                ...
                if (kick)
                    AmongUsClient.Instance.KickPlayer(player.GetClientId(), Options.AutoKickStartAsBan.GetBool());
                return true;
            }
        }
    ...
    public static bool Prefix(ChatController __instance)
    {
        ...
        switch (args[0])
        {
            ...
            case "/version":
                ...
                {
                    version_text +=
                        $"{kvp.Key}:{Utils.GetPlayerById(kvp.Key)?.Data?.PlayerName}:{kvp.Value.forkId}/{kvp.Value.version}({kvp.Value.tag})\n";
                }
        ...
        if (AmongUsClient.Instance.AmHost)
        {
            ...
            switch (args[0])
            {
                ...
                case "/winner":
                    canceled = true;
                    Utils.SendMessage("Winner: " +
                                      string.Join(",", Main.winnerList.Select(b => Main.AllPlayerNames[b])));
                    break;
                ...
                case "/hidename":
                    canceled = true;
                    Main.HideName.Value = args.Length > 1
                        ? args.Skip(1).Join(delimiter: " ")
                        : Main.HideName.DefaultValue.ToString();
                ...
                case "/messagewait":
                    ...
                    if (args.Length > 1 && int.TryParse(args[1], out int sec))
                    {
                        ...
                        Utils.SendMessage(
                            $"{GetString("Message.MessageWaitHelp")}\n{GetString("ForExample")}:\n{args[0]} 3", 0);
                ...
                case "/s":
                    canceled = true;
                    if (args.Length > 1)
                        Utils.SendMessage(args.Skip(1).Join(delimiter: " "),
                            title: $"<color=#ff0000>{GetString("MessageFromTheHost")}</color>");
                    break;
    ...
    {
        text = text.Trim();
        String target = new Il2CppSystem.String(' ', text.Length);
        LCMapString(0x0800, 0x02000000, text, text.Length, target, text.Length);
        return target;
        /*
    ...
    {
        /*
        for (int i = 0; i < Enum.GetValues(typeof(CustomRoles)).Length; i ++)
            RoleList.Add((CustomRoles)Enum.GetValues(typeof(CustomRoles)).GetValue(i), GetString(Enum.GetValues(typeof(CustomRoles)).GetValue(i).ToString()));*/
        foreach (CustomRoles value in Enum.GetValues(typeof(CustomRoles)))
        {
            RoleList.Add(value, GetString(value.ToString()));
    ...
    /*
    {
            ...
            { CustomRoles.MadSnitch, GetString("MadSnitch") },
            { CustomRoles.SKMadmate, GetString("SKMadmate") },
            //両陣営役職
    ...
    public static void SendRolesInfo(string role, PlayerControl player)
    {
        ...
        foreach (var r in RoleList)
        {
            ...
            if (String.Compare(role, roleName, StringComparison.OrdinalIgnoreCase) == 0 || String.Compare(role, roleShort, StringComparison.OrdinalIgnoreCase) == 0)
            {
